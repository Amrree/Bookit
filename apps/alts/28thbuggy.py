...
from full_wordpro_feed import FullWordProFeed
...
def fill_md_files(draft_path, story_prompt, model="llama3:70b", retries=3, status_callback=None):
    ...
    for idx, chapter_file in enumerate(chapter_files, 1):
        # Only fill in existing files; never create new ones or overwrite real content
        if not os.path.exists(chapter_file):
            if status_callback:
                status_callback(f"[SKIP] File does not exist: {chapter_file}")
            continue
        with open(chapter_file, 'r', encoding='utf-8') as f:
            content = f.read()
        # If file already has nontrivial content (more than just the header), skip
        lines = [line for line in content.splitlines() if line.strip()]
        if len(lines) > 1 and any(len(line.strip()) > 0 and not line.strip().startswith('#') for line in lines):
            previous_chapter = content[:1000]  # update for next prompt
            if status_callback:
                status_callback(f"[SKIP] {chapter_file} already has content, skipping.")
            continue
        # Find where to append: after any header, at first empty line
        ...
        try:
            new_content = ollama_generate(prompt, model=model, retries=retries)
            # Append to file (after header/blank lines)
            with open(chapter_file, 'a', encoding='utf-8') as f:
                # If file only has header, ensure a blank line before appending
                if not content.endswith('\n'):
                    f.write('\n')
                f.write(new_content.strip() + '\n')
            previous_chapter = new_content[:1000]
            if status_callback:
...
class EnhanceChaptersSidebar(tk.Frame):
    ...
    def trigger_enhance_chapter1(self):
        ...
        if not os.path.exists(chapter2_path):
            self.log_box.insert("end", f"[AUTO] Waiting for {chapter2_path} before enhancing chapter1.md...\n")
            self.log_box.see("end")
            return
        prompt_path = os.path.join(folder, "overview", "manifesto_prompt.txt")
    ...
    def run_manual(self):
        # Manual mode: enhance selected chapter only
        def enhance_thread():
            folder = self.get_project_folder()
            if not folder or not os.path.isdir(folder):
                self.log_box.insert("end", "[MANUAL] Please select a valid project folder first.\n")
                self.log_box.see("end")
                return
            drafts_dir = os.path.join(folder, "drafts")
            # Assume the user selects a chapter file in the UI; here, select the first as an example
            chapter_files = [f for f in os.listdir(drafts_dir) if f.startswith('chapter') and f.endswith('.md') and '_enhanced' not in f and '_combined' not in f]
            if not chapter_files:
                self.log_box.insert("end", "[MANUAL] No chapter files found in drafts folder.\n")
                self.log_box.see("end")
                return
            # TODO: Replace this with actual selection logic from the UI
            selected_chapter = os.path.join(drafts_dir, chapter_files[0])
            prompt_path = os.path.join(folder, "overview", "manifesto_prompt.txt")
            script_path = os.path.join(os.path.dirname(__file__), "03_enhance_chapter_manual.py")
            try:
                self.log_box.insert("end", f"[MANUAL] Enhancing {selected_chapter}...\n")
                self.log_box.see("end")
                result = subprocess.run([
                    "python3", script_path,
                ...
                self.log_box.insert("end", f"[MANUAL] Enhancement complete. Output:\n{result.stdout}\n")
                if result.stderr:
                    self.log_box.insert("end", f"[MANUAL] Enhancement errors:\n{result.stderr}\n")
                self.log_box.see("end")
            except Exception as e:
                self.log_box.insert("end", f"[MANUAL] Enhancement failed: {e}\n")
                self.log_box.see("end")
...
class BookstartApp:
    def __init__(self, root):
        ...
        root.grid_columnconfigure(2, weight=0, minsize=420)
        ...
        self.root.bind_all('<Control-w>', lambda event: self.launch_wordpro_feed())
    ...
    def add_save_menu(self):
        ...
        file_menu.add_separator()
        file_menu.add_command(label="Word Processor Mode", command=self.launch_wordpro_feed, accelerator="Ctrl+W")
    ...
    def launch_wordpro_feed(self):
        ...
        if not folder or not os.path.isdir(folder):
            ...
            return
        # Launch as a popup in the same process
        try:
            FullWordProFeed(self.root, project_folder=folder)
        except Exception as e:
            from tkinter import messagebox
            messagebox.showerror("Launch Failed", f"Could not launch Word Processor Mode.\n{e}")
...
def create_novel_folders_from_path(base_path, prompt=None):
    ...
    print(f"[CHECKPOINT] Creating book structure in '{base_path}'...")
    for folder in folders:
        os.makedirs(os.path.join(base_path, folder), exist_ok=True)
        print(f"[INFO] Created folder: {os.path.join(base_path, folder)}")
    drafts_path = os.path.join(base_path, "drafts")
    for i in range(1, 31):
        chapter_file = os.path.join(drafts_path, f"chapter{i}.md")
        with open(chapter_file, 'w', encoding='utf-8') as f:
            f.write(f"# Chapter {i}\n\n")
        print(f"[INFO] Created chapter file: {chapter_file}")
    structure_file = os.path.join(base_path, "folder_structure.txt")
    with open(structure_file, 'w', encoding='utf-8') as f:
        for folder in folders:
            f.write(f"{folder}\n")
    print(f"[INFO] Saved folder structure list to {structure_file}")
    prompt_file = os.path.join(base_path, "overview", "manifesto_prompt.txt")
    prompt_text = prompt if prompt else "[Add your book prompt here!]"
    with open(prompt_file, 'w', encoding='utf-8') as f:
        f.write(prompt_text)
    print(f"[INFO] Saved book prompt to {prompt_file}")
    return base_path
