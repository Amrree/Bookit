...
class BookstartApp:
    def __init__(self, root):
        ...
        self.start_btn.grid(row=5, column=0, columnspan=1, pady=(8, 0), sticky="ew")

        # Resume Button
        self.resume_btn = tk.Button(root, text="Resume", font=button_font, bg="#ffb700", fg="#161821", activebackground="#232946", activeforeground="#fff", relief='flat', padx=24, pady=10, borderwidth=0, highlightthickness=0, command=self.resume_generation)
        self.resume_btn.grid(row=5, column=1, columnspan=1, pady=(8, 0), sticky="ew")
    ...
    def resume_generation(self):
        """Resume chapter generation from the last filled chapter."""
        import importlib.util
        import sys
        folder = self.folder_var.get().strip()
        if not folder:
            self.status_var.set("No project folder selected.")
            self.log_terminal("[WARN] No project folder selected.")
            return
        drafts_dir = os.path.join(folder, "drafts")
        resume_path = os.path.join(os.path.dirname(__file__), "resume.py")
        spec = importlib.util.spec_from_file_location("resume", resume_path)
        resume = importlib.util.module_from_spec(spec)
        sys.modules["resume"] = resume
        spec.loader.exec_module(resume)
        next_chap = resume.get_next_chapter_to_generate(drafts_dir)
        self.status_var.set(f"Resuming at chapter {next_chap}...")
        self.log_terminal(f"[INFO] Resuming at chapter {next_chap}...")
        # Now call the generation logic for this chapter (simulate pressing start for that chapter)
        # You may want to call your fill_md_files or similar, passing next_chap as a start point
        # For now, just log the info
        # Optionally: Implement more granular resume logic here
